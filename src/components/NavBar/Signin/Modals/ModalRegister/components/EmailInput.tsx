...
const EmailInput: React.FC<DataInputProps> = ({ setErroRegisterUser, errorRegisterUser, setUserLogin, userLogin }) => {
  const { mode } = useThemeMode();
  const colors = tokens(mode);
  const handleChangeEmail = (e: ChangeEvent<HTMLInputElement>) => {
    setErroRegisterUser({ ...errorRegisterUser, errorEmail: false });
    e.preventDefault();
